...
```bash
rg -n "anchors (remain)|fallback (anchors)|current (anchors) only|stable (anchors)" docs/interview-memory-clean-architecture-plan.md
perl -ne 'print "$.:$_" if /\x{2014}/' docs/interview-memory-clean-architecture-plan.md
```
