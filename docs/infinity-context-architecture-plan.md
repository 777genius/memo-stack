# Universal Infinity Context Architecture Plan

Статус: Draft architecture decision, готово к review перед реализацией.

Дата фиксации: 2026-05-24.

Цель документа: зафиксировать целевую архитектуру reusable infinity context, которую можно подключать к Client App и другим приложениям без привязки к Graphiti, Qdrant, Zep, Cognee или конкретному LLM provider.

Ключевое решение:

```text
Our Infinity Context Core + Postgres canonical truth + Graphiti adapter + Qdrant adapter
```
...
Мы не делаем "интеграцию с Graphiti" и не делаем "интеграцию с Qdrant". Мы делаем свою **Infinity Context**, где:

- `infinity_context_core` содержит domain, use cases и ports.
- `infinity_context_server` дает HTTP API, worker, migrations, auth, config.
- `infinity_context_adapters` подключают Postgres, Graphiti, Qdrant, LLM, embeddings, object storage.
- `infinity_context_sdk` дает удобный Python client, потом можно сгенерировать TypeScript/Rust clients из OpenAPI.
- `infinity_context_mcp` и UI добавляются позже поверх того же API.
...
67. Public DTO/schema ownership was still implicit.
    ...
    - Fix: add a versioned `infinity_context_contracts` package with canonical public DTOs, JSON schemas, fixtures and schema registry.
...
|---|---|---|
| Clean boundaries | Architectural Principles, Package Layout, Ports, First Concrete PR | `infinity_context_core` must not import infrastructure clients or FastAPI |
...
| Canonical contract schemas | keeps API, SDK, events, bundles and MCP tools aligned | `infinity_context_contracts`, schema registry, DTO mappers | schema roundtrip, unknown enum and OpenAPI fixture tests | clients drift or import/export becomes incompatible |
...
- composition root живет в `infinity_context_server`.
...
```mermaid
flowchart TD
  A["Client App: Client App, Codex, Claude, Slack"] --> B["infinity_context_sdk or HTTP API"]
  B --> C["infinity_context_server: FastAPI"]
...
```text
client -> infinity_context_server API -> use case -> canonical Postgres transaction -> outbox -> async indexes
```
...
- adapter dependencies stay out of `infinity_context_core`;
...
- `infinity_context_contracts` owns public DTOs, JSON schema ids, fixture examples and version metadata;
- `infinity_context_core` owns domain entities, value objects, use-case commands and domain errors;
- `infinity_context_server` maps HTTP requests into application commands and maps results into public DTOs;
- `infinity_context_sdk` consumes generated or shared contract models, but does not import domain entities;
...
```text
infinity_context_core/domain imports infinity_context_contracts: forbidden
infinity_context_contracts imports FastAPI/SQLAlchemy/Graphiti/Qdrant: forbidden
infinity_context_server imports infinity_context_contracts: allowed
infinity_context_sdk imports infinity_context_contracts or generated models: allowed
```
...
```text
No infinity_context_server process accepts normal mutating/read API traffic until bootstrap, migrations and runtime config validation are complete.
```
...
```text
infinity_context_server bootstrap status
infinity_context_server bootstrap init-local --owner-name ... --space-name ... --memory-scope-name ...
infinity_context_server bootstrap create-token --scope local_setup --ttl 10m
infinity_context_server config doctor
infinity_context_server dev seed --synthetic-only
infinity_context_server dev reset --confirm-local-lineage ...
```
...
```text
infinity_context_server migrate plan
infinity_context_server migrate dry-run
infinity_context_server migrate apply --phase expand
infinity_context_server migrate backfill --job ...
infinity_context_server migrate verify
infinity_context_server migrate cleanup --after-compat-window
...
- `local_fallback_only` and `no_memory` are valid safe outcomes when infinity_context_server cannot answer in time;
...
- local fallback is client-owned and must be clearly distinguished from infinity_context_server context.
...
- infinity_context_server down still produces local/minimal fallback in desktop flow;
...
Actor that calls infinity_context_server.
...
Not acceptable for a reusable team infinity context because one repository method or maintenance job can bypass endpoint-level guardrails.
...
```text
project "Infinity Context" in Client App architecture memory scope
package "infinity_context_core" in repo Client App
...
ingested_at:
  when infinity_context_server stored the record
...
- they may be served by infinity_context_server during migration;
...
| Canonical DTO schemas | `infinity_context_contracts` schema registry | public DTOs are additive within supported version window | schema ADR, fixture update and SDK compatibility evidence |
...
1. Compatibility gateway first
   ...
   Infinity context implements legacy `/api/v1/interview-memory/*` routes as an adapter over the new core. Client App can point `INTERVIEW_MEMORY_API_URL` to local platform without desktop bridge rewrite.
...
```text
1. Add compatibility routes in infinity_context_server.
2. Run existing Client App canary against infinity_context_server by changing only INTERVIEW_MEMORY_API_URL.
3. Add new SDK client behind the same desktop mode resolver.
4. Shadow compare old backend vs infinity_context_server retrieval with production-safe diffs.
...
Infinity context processes untrusted text and files. Dependency governance is therefore part of product safety, not a CI afterthought. This includes Graphiti, Qdrant client, Neo4j driver, FastAPI stack, LLM/provider SDKs, parser/OCR libraries, archive readers and object storage SDKs.
...
Recommended. It is strong enough for local/team v1 without turning the infinity context into a full security product.
...
Too weak for a infinity context that ingests arbitrary files and routes content to AI providers.
...
```text
docker compose up infinity_context_server postgres qdrant neo4j object_storage worker
...
|---|---|---|
| Rebuild Qdrant | `infinity_context_server rebuild vector --memory_scope ...` | Postgres remains canonical, deleted chunks stay hidden |
| Rebuild Graphiti | `infinity_context_server rebuild graph --memory_scope ...` | facts reindexed from active canonical records only |
| Create backup | `infinity_context_server backup create --policy ...` | manifest records commit cursor, key versions and object inventory hash |
| Validate restore | `infinity_context_server restore validate --manifest ...` | reports key/object/tombstone blockers without mutation |
...
| Secret remediation | `infinity_context_server remediate secrets --source ...` | stale embeddings deindexed, redacted artifacts created, audit safe |
...
"infinity_context_core" package in Client App vs another repo with same package name.
```
...
- local infinity_context_server can run in Docker;
...
```text
infinity_context_server starts in a fresh DB and accepts normal API requests before owner/token setup.
```
...
- generated SDK smoke tests call a real infinity_context_server test instance;
...
- public routes return only `infinity_context_contracts` DTOs or generated equivalents;
...
- all external LLM/embedding providers are configured through infinity_context_server config and `MemoryPolicy`;
...
```text
old local_interview_memory Postgres and new infinity_context_server local Docker both ingest the same events.
```

Expected behavior:

- integration mode chooses one write target per run;
- diagnostics report selected memory target: legacy_backend, local_db, infinity_context_server or disabled;
...
```text
Postgres + Qdrant + Graphiti backend + infinity_context_server
```
...
- `infinity_context_contracts` schema registry matches OpenAPI components and JSON schema artifacts;
...
- verify old SDK/client fixture can read response from current infinity_context_server without crashing on additive fields;
...
Minimum infinity_context checks:

```bash
pytest packages/infinity_context_core/tests packages/infinity_context_server/tests
pytest packages/infinity_context_core/tests/test_fact_currency.py packages/infinity_context_server/tests/test_current_fact_context.py
pytest packages/infinity_context_core/tests/test_backup_restore.py packages/infinity_context_server/tests/test_restore_validation.py
pytest packages/infinity_context_core/tests/test_visibility_guard.py packages/infinity_context_server/tests/test_read_visibility.py
pytest packages/infinity_context_core/tests/test_prompt_path_slo.py packages/infinity_context_server/tests/test_context_deadlines.py
pytest packages/infinity_context_core/tests/test_cache_freshness.py packages/infinity_context_server/tests/test_cache_invalidation.py
pytest packages/infinity_context_core/tests/test_data_residency.py packages/infinity_context_server/tests/test_processing_location.py
pytest packages/infinity_context_core/tests/test_repository_scope.py packages/infinity_context_server/tests/test_query_scope.py
pytest packages/infinity_context_core/tests/test_supply_chain.py packages/infinity_context_server/tests/test_parser_sandbox.py
python -m infinity_context_server openapi > /tmp/memory-openapi.json
python -m infinity_context_server diagnostics --check-supply-chain --check-parser-sandbox
python -m infinity_context_server diagnostics --check-indexes --check-capabilities
python -m infinity_context_server worker --once --queue maintenance
...
- packer drops.
- selected memory target: disabled, legacy_backend, local_db, infinity_context_server;
...
Infinity context rollout must be reversible at each layer.
...
| Public DTO schema drifts from domain/use cases | 9 | 5 | `infinity_context_contracts`, mapper tests, golden fixtures, OpenAPI diff |
...
V1 target is a reusable infinity context that can run locally and be integrated safely. V1 is not a full SaaS and not a full Zep competitor.
...
- no Graphiti/Qdrant adapter until `infinity_context_core` import boundaries are enforced by tests;
- no public API expansion until `infinity_context_contracts` schema registry and mapper tests exist;
...
- no prompt-impacting Client App switch until compatibility canary and document recall pass against infinity_context_server;
...
1. Core skeleton and contracts
   ...
   - package layout;
   - `infinity_context_contracts` package and schema registry skeleton;
...
13. Client App compatibility gateway
   ...
   - existing canary scripts against infinity_context_server.
...
🎯 9   🛡️ 9   🧠 7
Estimated changes: `900-1800` lines across infinity_context_server compatibility routes, tests and small Client App config wiring.

Deliver:

- legacy `/api/v1/interview-memory/*` compatibility routes in infinity_context_server;
- mapping from legacy `session_id` to platform `MemoryThread`;
- response envelope compatible with current desktop bridge;
- infinity_context_server URL config through existing `INTERVIEW_MEMORY_API_URL` path first;
...
- fallback works when infinity_context_server down.
- existing mode precedence tests pass;
- existing canary scripts pass against infinity_context_server URL;
...
**Our Infinity Context Core + Graphiti + Qdrant directly**
🎯 9   🛡️ 9   🧠 7
...
1. No Graphiti/Qdrant types in `infinity_context_core`.
...
72. Public DTOs live in `infinity_context_contracts`, not in domain entities, ORM models or adapter payloads.
...
7. Package/repository location.
   - Recommendation: start inside Client App `infinity_context/` until architecture is proven, then extract to standalone repo/package.
...
12. AI provider gateway scope.
    ...
    - Alternative: expose a generic model-call API in `infinity_context_core`, but this increases coupling and misuse risk.
...
26. Contract model implementation style.
    - Recommendation: separate `infinity_context_contracts` package with public DTOs/schema registry, while `infinity_context_core` keeps dependency-light domain entities and internal use-case DTOs.
    - Alternative: put all DTOs inside `infinity_context_core`, but this makes domain refactors and public contract evolution harder to separate.
...
```text
feat(memory): scaffold reusable infinity context core and server
```

Scope:

- `infinity_context_contracts` package;
- `infinity_context_core` package;
- domain entities;
...
- `infinity_context_core` imports only stdlib/domain-safe dependencies;
- `infinity_context_contracts` imports no FastAPI, SQLAlchemy, Graphiti, Qdrant or domain infrastructure;
- `infinity_context_server` owns FastAPI/composition root;
...
```bash
pytest packages/infinity_context_core/tests packages/infinity_context_server/tests
pytest packages/infinity_context_contracts/tests
python -m infinity_context_server openapi > /tmp/memory-openapi.json
rg -n "graphiti|qdrant_client|sqlalchemy|fastapi" packages/infinity_context_core
rg -n "fastapi|sqlalchemy|graphiti|qdrant_client" packages/infinity_context_contracts
```

The import-boundary commands should return no forbidden imports from `infinity_context_core` or `infinity_context_contracts`.
...
- public DTOs live in `infinity_context_contracts` and are covered by schema registry, fixtures and mapper tests;
