# Infinity Context Core Lite Implementation Plan

Статус: рабочий план реализации Core Lite. Глобальный reference план остается в `docs/infinity-context-architecture-plan.md`.

Дата фиксации: 2026-05-24.

## Summary

Core Lite - это не полный SaaS/Zep competitor. Это минимальная reusable infinity context, которую можно быстро подключить к Client App, а потом вынести в отдельный сервис/SDK для других проектов.

Главное решение:

```text
Infinity Context Core Lite = Postgres canonical truth + Qdrant RAG + thin Graphiti adapter + Client App compatibility gateway
```
...
1. Разделить domain, application, ports, adapters и server так, чтобы `infinity_context_core` можно было вынести в отдельную библиотеку.
...
8. `infinity_context_core` cannot import infrastructure.
...
1. `infinity_context_core` без инфраструктурных зависимостей.
2. `infinity_context_server` HTTP API и composition root.
...
```text
infinity_context_core imports fastapi
infinity_context_core imports sqlalchemy
infinity_context_core imports qdrant_client
infinity_context_core imports graphiti
...
- `infinity_context_server` wires concrete implementations.
...
infinity_context_core.ports -> typing/protocols + domain DTOs
infinity_context_adapters -> infinity_context_core ports + external libraries
infinity_context_server -> infinity_context_core application + adapters
infinity_context_sdk -> HTTP DTOs only
```
...
def test_memory_core_has_no_infrastructure_imports():
    imports = scan_imports("infinity_context/packages/infinity_context_core")
...
```text
infinity_context_core has no infrastructure imports
infinity_context_core has no environment variable reads
infinity_context_core has no wall-clock direct calls except through ClockPort
infinity_context_core has no uuid direct calls except through IdGeneratorPort
infinity_context_server routes do not instantiate repositories directly
infinity_context_adapters do not import FastAPI routes
...
def test_use_cases_depend_on_ports_not_adapters():
    imports = scan_imports("infinity_context/packages/infinity_context_core/infinity_context_core/application")
    forbidden = {
        "infinity_context_adapters",
        "sqlalchemy",
        "qdrant_client",
...
def test_routes_do_not_open_database_sessions_directly():
    source = read_all_python("infinity_context/packages/infinity_context_server/infinity_context_server/api")
...
Adding another client app does not change infinity_context_core.
...
No use case imports infinity_context_adapters, FastAPI, SQLAlchemy, Qdrant or Graphiti.
```
...
- let `infinity_context_server` bypass use cases and write repositories directly.
...
Allowed exceptions across `infinity_context_core` boundaries:
...
Not allowed across `infinity_context_core` boundaries:
...
- candidate DTOs live in `infinity_context_core.ports`, not in provider adapter packages.
...
```text
infinity_context/packages/infinity_context_core
infinity_context/packages/infinity_context_server
infinity_context/packages/infinity_context_adapters
```
...
`infinity_context_core` public exports:
...
`infinity_context_server` public surface:
...
`infinity_context_adapters` public surface:
...
- `infinity_context_sdk` imports only HTTP/client models, not `infinity_context_core`;
- Client App imports only `infinity_context_sdk` or uses HTTP;
- public API changes require changelog entry in the plan or ADR during implementation.

Example `infinity_context_core/application/__init__.py`:

```python
from infinity_context_core.application.use_cases.remember_fact import (
    RememberFactCommand,
    RememberFactUseCase,
)
...
```bash
mkdir -p infinity_context/packages/infinity_context_core/infinity_context_core/{domain,application,ports}
mkdir -p infinity_context/packages/infinity_context_server/infinity_context_server/api/v1
mkdir -p infinity_context/packages/infinity_context_adapters/infinity_context_adapters/{noop,postgres,qdrant,graphiti,embeddings}
mkdir -p infinity_context/packages/infinity_context_sdk/infinity_context_sdk
mkdir -p infinity_context/tests/{unit,integration,e2e,fixtures}
mkdir -p docs/adr
```
...
```text
infinity_context/pyproject.toml
infinity_context/docker-compose.yml
infinity_context/.env.example
infinity_context/packages/infinity_context_core/pyproject.toml
infinity_context/packages/infinity_context_core/infinity_context_core/domain/entities.py
infinity_context/packages/infinity_context_core/infinity_context_core/domain/errors.py
infinity_context/packages/infinity_context_core/infinity_context_core/ports/*.py
infinity_context/packages/infinity_context_server/infinity_context_server/main.py
infinity_context/packages/infinity_context_server/infinity_context_server/config.py
infinity_context/packages/infinity_context_server/infinity_context_server/composition.py
infinity_context/packages/infinity_context_adapters/infinity_context_adapters/noop/*.py
infinity_context/tests/unit/test_import_boundaries.py
docs/adr/ADR-0001-infinity-context-core-lite-boundaries.md
...
```text
python -m infinity_context_server.main starts
...
6. E2E canary before enabling active context against infinity_context_server.
...
```text
infinity_context_core/domain/entities.py
infinity_context_core/application/use_cases/remember_fact.py
infinity_context_core/ports/repositories.py
infinity_context_core/ports/unit_of_work.py
infinity_context_server/api/v1/facts.py
infinity_context_server/mappers.py
infinity_context_adapters/postgres/fact_repository.py
infinity_context_adapters/postgres/unit_of_work.py
...
```bash
pytest infinity_context/tests/unit/test_memory_fact.py
pytest infinity_context/tests/application/test_remember_fact.py
pytest infinity_context/tests/integration/test_postgres_fact_repository.py
```
...
```bash
pytest infinity_context/tests/application/test_forget_fact.py
pytest infinity_context/tests/e2e/test_forget_hides_context.py
```
...
```bash
pytest infinity_context/tests/application/test_ingest_document.py
pytest infinity_context/tests/e2e/test_document_recall.py
```
...
```bash
pytest infinity_context/tests/e2e/test_legacy_client_context.py
...
- API request DTOs live in `infinity_context_server`;
- application command/query DTOs live in `infinity_context_core.application`;
...
ingested_at
  when infinity_context_server accepted it, server clock only
...
```python
# packages/infinity_context_core/infinity_context_core/ports/repositories.py
from typing import Protocol
from infinity_context_core.domain.entities import MemoryFact, MemoryFactId, MemoryScopeId, SpaceId
...
```python
# packages/infinity_context_core/infinity_context_core/application/use_cases/remember_fact.py
from dataclasses import dataclass
from infinity_context_core.domain.entities import MemoryFact, FactStatus, SourceRef
from infinity_context_core.ports.repositories import FactRepositoryPort
...
```python
# packages/infinity_context_core/infinity_context_core/ports/indexes.py
from typing import Protocol
...
```python
# packages/infinity_context_server/infinity_context_server/api/v1/facts.py
...
```python
# packages/infinity_context_server/infinity_context_server/api/legacy_client.py
...
```python
# packages/infinity_context_server/infinity_context_server/mappers.py
...
```python
# packages/infinity_context_adapters/infinity_context_adapters/postgres/fact_repository.py
...
- no module-level singleton provider client in `infinity_context_core`;
- no route should import `infinity_context_adapters.qdrant` or `infinity_context_adapters.graphiti`.
...
```python
# packages/infinity_context_core/infinity_context_core/domain/errors.py
...
```python
# packages/infinity_context_server/infinity_context_server/api/errors.py
...
```python
from infinity_context_sdk import MemoryClient
...
- SDK does not import `infinity_context_core`;
...
```text
infinity_context_adapters/postgres/models.py
  SQLAlchemy table/ORM definitions only

infinity_context_adapters/postgres/mappers.py
  ORM/domain conversion only

infinity_context_adapters/postgres/repositories.py
...
- no `Legacy*Request` type can be imported by `infinity_context_core`;
- no Client App env var is read inside `infinity_context_core`;
...
- Pydantic models are not imported by `infinity_context_core`;
...
sequenceDiagram
  participant Client
  participant API as infinity_context_server API
...
```text
MEMORY_DATABASE_URL=postgresql+asyncpg://infinity_context:infinity_context@127.0.0.1:54329/infinity_context
...
- every provider dependency lives in `infinity_context_adapters`, not `infinity_context_core`;
...
services:
  infinity_context_postgres:
    image: postgres:16
    environment:
      POSTGRES_USER: infinity_context
      POSTGRES_PASSWORD: infinity_context
      POSTGRES_DB: infinity_context
    ports:
      - "54329:5432"
  ...
  infinity_context_server:
    build: .
    environment:
      MEMORY_DATABASE_URL: postgresql+asyncpg://infinity_context:infinity_context@infinity_context_postgres:5432/infinity_context
      MEMORY_SERVICE_TOKEN: local-dev-token
      MEMORY_QDRANT_URL: http://infinity_context_qdrant:6333
      MEMORY_QDRANT_ENABLED: "true"
      MEMORY_GRAPHITI_ENABLED: "false"
      MEMORY_EMBEDDINGS_ENABLED: "false"
    depends_on:
...
Use Alembic or equivalent migration tooling inside `infinity_context_adapters.postgres`.
...
- migrations do not import `infinity_context_core`;
...
```bash
python -m infinity_context_server.admin backfill --name chunker-version-v1 --batch-size 500 --dry-run
python -m infinity_context_server.admin backfill --name chunker-version-v1 --batch-size 500
```
...
```bash
python -m infinity_context_server.admin reset-local --i-understand-this-deletes-local-memory
```
...
python -m infinity_context_server.worker --loop
```
...
```bash
python -m infinity_context_server.admin reindex-qdrant --space client-app --memory_scope default
python -m infinity_context_server.admin reindex-graphiti --space client-app --memory_scope default
python -m infinity_context_server.admin repair-projections --space client-app --dry-run
...
```text
infinity_context_chunks_v1_1536
...
| `local_only` | local fallback only | infinity_context_server avoided | local evidence only |
...
- rate limiting belongs to `infinity_context_server`, not domain;
...
- document ingest can return retryable `memory.backpressure` when
  ...
  the threshold;
- backpressure is implemented in `infinity_context_server`, not domain/use cases;
...
- provider budgets are disabled by default and live in `infinity_context_server` wiring/worker code,
...
- import-boundary tests prove `infinity_context_core` has no FastAPI/SQLAlchemy/provider SDK imports,
...
```text
ruff format/check
type check for infinity_context_core and infinity_context_server
import-boundary test
no forbidden imports in infinity_context_core
...
- `make infinity-context-test-quality` is the local full quality gate and runs lint,
  application tests, integration tests, e2e-style memory tests and golden eval;
- `make infinity-context-test-application` covers domain/fact/context/suggestion behavior;
- `make infinity-context-test-integration` covers provider adapters and worker retry/dead-letter
  behavior;
- `make infinity-context-test-e2e` covers legacy/context and worker paths used by the live memory
  workflow;
- `make infinity-context-eval` runs the small golden suite and prompt contract snapshot check;
...
- package layout;
- `infinity_context_core` domain entities and ports;
- `infinity_context_server` FastAPI app skeleton;
- `infinity_context_adapters` empty adapters;
...
1. Create `infinity_context/`.
2. Add package pyprojects.
3. Add domain dataclasses and ports.
4. Add FastAPI app with health/capabilities.
5. Add tests ensuring `infinity_context_core` has no forbidden imports.

Exit gate:

```bash
pytest infinity_context/packages/infinity_context_core/tests
pytest infinity_context/packages/infinity_context_server/tests
rg -n "fastapi|sqlalchemy|qdrant|graphiti" infinity_context/packages/infinity_context_core
```
...
```bash
pytest infinity_context/packages/infinity_context_core/tests/test_fact_lifecycle.py
pytest infinity_context/packages/infinity_context_adapters/tests/test_postgres_fact_repository.py
```
...
```bash
pytest infinity_context/packages/infinity_context_adapters/tests/test_qdrant_adapter.py
pytest infinity_context/tests/e2e/test_document_recall.py
```
...
pytest infinity_context/packages/infinity_context_core/tests/test_context_builder.py
```
...
```bash
pytest infinity_context/packages/infinity_context_adapters/tests/test_graphiti_adapter.py
pytest infinity_context/tests/integration/test_graph_context.py
```
...
- Python SDK;
- `.env.example`;
- Docker compose: infinity_context_server, Postgres, Qdrant, Neo4j if Graphiti phase enabled;
...
- `docker-compose.yml` starts `infinity_context_server`, Postgres, Qdrant and Neo4j for local use;
- Docker startup runs `infinity_context_server.db upgrade` and `infinity_context_server.admin seed-defaults`
  before serving HTTP;
- Docker Compose waits for Postgres health before starting `infinity_context_server` and exposes a
  server `/v1/health` healthcheck;
- `.env.example` contains local defaults for server, embeddings, Qdrant, Graphiti and
  Client App compatibility;
- `make infinity-context-up` starts the local server service through Docker Compose;
- `make infinity-context-smoke` starts the local server service, waits for `/v1/health` and runs
  the SDK smoke path;
- `make infinity-context-api-smoke` runs `examples/integration_memory_smoke.py` against
  `http://127.0.0.1:7788` by default;
- the smoke script uses only the Infinity Context SDK client and verifies health, space/memory scope
...
Goal: Client App should treat infinity_context_server as an external dependency and keep its current local fallback.

Integration sequence:

1. Add infinity_context_server local Docker runbook.
2. Add `INTERVIEW_MEMORY_API_URL=http://127.0.0.1:7788` for local canary.
3. Keep existing `INTERVIEW_MEMORY_MODE` behavior.
4. Route only memory calls to infinity_context_server.
...
| Client App field | Infinity Context field |
|---|---|
...
```text
Any network/auth/timeout/schema error from infinity_context_server -> local fallback.
...
| Phase 1 Postgres facts | API only | stop infinity_context_server | fact lifecycle tests |
...
1. Run infinity_context_server in shadow ingest.
...
The Infinity Context is implemented in its own repository:

```text
/Users/belief/dev/projects/ai/infinity-context
https://github.com/777genius/infinity-context.git
```
...
- Client App integration stays client-side through HTTP/SDK/MCP and must not
  import provider adapters from Infinity Context.
...
Use this playbook when implementing the next Infinity Context architecture work
...
```bash
cd /Users/belief/dev/projects/ai/infinity-context
git status --short
df -h .
make infinity-context-lint
make infinity-context-test-application
...
5. Export new contracts from `infinity_context_core.ports`.
6. Add tests that prove `infinity_context_core` imports no Cognee, Graphiti, Qdrant,
...
```text
packages/infinity_context_core/infinity_context_core/application/use_cases/get_capabilities.py
...
packages/infinity_context_server/infinity_context_server/api/v1/legacy_client.py
...
make infinity-context-lint
make infinity-context-test-integration
make infinity-context-worker-once
```
...
- Cognee SDK leaks into `infinity_context_core`;
...
- Cognee output becomes canonical fact without Infinity Context Core approval;
...
packages/infinity_context_core/infinity_context_core/application/use_cases/ingest_episode.py
packages/infinity_context_server/infinity_context_server/api/v1/suggestions.py
packages/infinity_context_mcp/infinity_context_mcp/application/service.py
...
- `infinity_context_core.ports.auth` defines `MemoryUser`, `MemoryScope` and `ReadScope`
...
- the MCP adapter uses a dedicated read scope for search/context so multi-memory-scope
  `memory_scope_external_refs` are sent to Infinity Context Server instead of being silently reduced
  to one memory scope;
...
- No provider SDK import crosses into `infinity_context_core`.
- Every derived hit is hydrated before prompt rendering.
- Prompt memory is evidence, never instruction.
- Client App imports no Infinity Context internals.
...
```bash
python -m infinity_context_server.doctor --gate active_context
```
...
- `python -m infinity_context_server.doctor --gate active_context` checks doctor status, default
...
```bash
python -m infinity_context_server.admin token create --space client-app --description local-dev
python -m infinity_context_server.admin token revoke --token-id tok_123
python -m infinity_context_server.admin token list --space client-app
```
...
- `python -m infinity_context_server.admin compact-outbox` compacts old `done` outbox payloads
...
### Client App Infinity Context Server Timeout
...
| infinity_context_server down | interview blocked | desktop local fallback continues | Client App canary |
...
```text
infinity_context/tests/unit/test_memory_fact.py
infinity_context/tests/unit/test_memory_document.py
infinity_context/tests/unit/test_policy_admission.py
infinity_context/tests/unit/test_context_packer.py
infinity_context/tests/unit/test_id_generation.py
infinity_context/tests/application/test_remember_fact.py
infinity_context/tests/application/test_update_fact.py
infinity_context/tests/application/test_forget_fact.py
infinity_context/tests/application/test_ingest_document.py
infinity_context/tests/application/test_build_context.py
infinity_context/tests/application/test_scope_guard.py
infinity_context/tests/integration/test_postgres_fact_repository.py
infinity_context/tests/integration/test_postgres_document_repository.py
infinity_context/tests/integration/test_outbox_worker.py
infinity_context/tests/integration/test_qdrant_adapter_contract.py
infinity_context/tests/integration/test_graphiti_adapter_contract.py
infinity_context/tests/e2e/test_document_recall.py
infinity_context/tests/e2e/test_fact_update.py
infinity_context/tests/e2e/test_forget.py
infinity_context/tests/e2e/test_legacy_client_context.py
infinity_context/tests/e2e/test_prompt_injection.py
infinity_context/tests/e2e/test_cross_memory scope_isolation.py
```
...
- `python -m infinity_context_server eval run --suite small-golden` seeds deterministic
  local SQLite fixtures with facts, documents, update/delete, prompt-injection
  and cross-memory-scope cases;
- `--api-url` runs the same suite against a live Infinity Context HTTP server;
...
python -m infinity_context_server eval snapshots --suite prompt-contract --update
```
...
- `python -m infinity_context_server eval snapshots --suite prompt-contract`;
- `python -m infinity_context_server eval snapshots --suite prompt-contract --update`;
- `make infinity-context-eval` runs both `small-golden` and `prompt-contract`;
...
```bash
python -m infinity_context_server eval run --suite small-golden --api-url http://127.0.0.1:7788
```
...
pytest infinity_context/packages/infinity_context_core/tests
pytest infinity_context/packages/infinity_context_server/tests
pytest infinity_context/packages/infinity_context_adapters/tests
```
...
make infinity-context-doctor
make infinity-context-worker-once
make infinity-context-up
make infinity-context-down
```
...
infinity-context-test-unit:
	pytest infinity_context/tests/unit
...
cd infinity_context
cp .env.example .env
docker compose up -d infinity_context_postgres infinity_context_qdrant
...
```bash
python -m infinity_context_server.admin check-invariants --space client-app --memory_scope default
python -m infinity_context_server.admin check-invariants --space client-app --memory_scope default --include-projections
```
...
```bash
python -m infinity_context_server.admin export-memory_scope --space client-app --memory_scope default --out memory_scope-export.json
python -m infinity_context_server.admin import-memory_scope --space client-app --memory_scope default --file memory_scope-export.json --dry-run
```
...
```bash
python -m infinity_context_server.admin repair-projections --space client-app --memory_scope default --dry-run
python -m infinity_context_server.admin reindex-qdrant --space client-app --memory_scope default
```
...
```bash
MEMORY_GRAPHITI_ENABLED=false python -m infinity_context_server.main
python -m infinity_context_server.admin reindex-graphiti --space client-app --memory_scope default --dry-run
```
...
```text
infinity_context_core domain/application ports
public /v1 API DTOs
SDK method names
...
- reusable boundaries live in `infinity_context_core` ports/use cases, public `/v1` DTOs, Python SDK,
...
- if infinity_context_server fails, Client App uses local/current-session fallback.
...
- Client App can point to infinity_context_server and pass current memory canaries;
...
```text
feat(memory): scaffold core-lite infinity context
```
...
- domain entities;
- ports;
- `infinity_context_server` health/capabilities;
- empty adapters;
- import-boundary tests;
...
infinity_context/pyproject.toml
infinity_context/packages/infinity_context_core/pyproject.toml
infinity_context/packages/infinity_context_core/infinity_context_core/domain/*
infinity_context/packages/infinity_context_core/infinity_context_core/ports/*
infinity_context/packages/infinity_context_server/pyproject.toml
infinity_context/packages/infinity_context_server/infinity_context_server/main.py
infinity_context/packages/infinity_context_server/infinity_context_server/config.py
infinity_context/packages/infinity_context_adapters/pyproject.toml
infinity_context/tests/unit/test_import_boundaries.py
docs/adr/ADR-0001-infinity-context-core-lite-boundaries.md
```
...
infinity_context_core has no forbidden imports
unit tests pass
...
infinity_context_core/application/use_cases/remember_fact.py
infinity_context_core/application/use_cases/update_fact.py
infinity_context_core/application/use_cases/forget_fact.py
infinity_context_server/api/v1/facts.py
```
...
```text
infinity_context_server/api/legacy_client.py
infinity_context_core/application/use_cases/ingest_episode.py
infinity_context_core/application/use_cases/build_context.py
...
infinity_context_server timeout falls back locally
...
```text
infinity_context_core/application/use_cases/ingest_document.py
infinity_context_core/application/use_cases/process_document.py
infinity_context_adapters/qdrant/vector_adapter.py
infinity_context_adapters/embeddings/noop_adapter.py
infinity_context_adapters/embeddings/openai_adapter.py
```
...
```text
Does infinity_context_core avoid infrastructure imports?
...
1. Create `infinity_context/pyproject.toml`.
2. Create `infinity_context_core` package with `domain`, `application`, `ports`.
3. Add minimal domain value objects and errors.
4. Add empty ports with `Protocol`.
5. Create `infinity_context_server` with `/v1/health` and `/v1/capabilities`.
6. Create `infinity_context_adapters` package with noop adapters only.
...
```bash
pytest infinity_context/tests/unit/test_import_boundaries.py
python -m infinity_context_server.main
curl http://127.0.0.1:7788/v1/health
```
...
```text
infinity_context_core domain/use cases/ports: 5800-9800
infinity_context_server API/composition/config/auth/admin: 4300-7600
