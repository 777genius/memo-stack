...
| Context Assembly v2 | Retrieval sources bounded, ranking reasons present, score signals redacted, provenance normalized, hybrid and temporal counters present, prompt-ready citations and evidence locations gated by scorecard | `tests/unit/test_context_diagnostics.py`, `tests/unit/test_context_packer.py`, `tests/unit/test_context_provider_consistency.py`, SDK diagnostics tests, context hydrator batch lookup tests, `.e2e-artifacts/memory-quality-scorecard.json` | Strong | Add new call-count guards when introducing expensive rerankers or new derived collectors |
