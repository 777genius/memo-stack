...
Core Lite - это не полный SaaS/Zep competitor. Это минимальная reusable memory platform, которую можно быстро подключить к HackInterview, а потом вынести в отдельный сервис/SDK для других проектов.

Главное решение:

```text
Memory Core Lite = Postgres canonical truth + Qdrant RAG + thin Graphiti adapter + HackInterview compatibility gateway
```
...
- HackInterview сейчас нужен reliable recall, update/forget и prompt context, а не enterprise compliance.
...
3. Сначала доказать correctness для HackInterview: document recall, fact update, forget, fallback.
...
7. HackInterview must keep fallback behavior when memory server fails.
...
9. Context builder для HackInterview prompt path.
...
### Option A - HackInterview-first Core Lite

🎯 9   🛡️ 8   🧠 6
Approx changes: `22000-42000` lines.

Рекомендую. Делаем отдельный Python memory service, но первый compatibility target - HackInterview. Сразу держим Clean Architecture и ports, но режем enterprise guardrails.

### Option B - Embedded HackInterview backend feature

🎯 8   🛡️ 7   🧠 5
Approx changes: `5000-9000` lines.

Быстрее, но хуже reuse. Получим memory только для HackInterview, а потом придется вынимать в отдельную платформу.
...
Архитектурно сильнее, но слишком широкий первый шаг. Риск застрять до реального полезного recall в HackInterview.
...
HackInterview imports graphiti
HackInterview imports qdrant_client
...
HackInterview does not import provider SDKs
...
legacy HackInterview session maps to one active thread
...
- HackInterview imports only `memory_sdk` or uses HTTP;
...
4. Context builder before prompt-impacting HackInterview switch.
...
```text
install Graphiti -> write adapter -> change HackInterview prompt -> add tests later
```
...
commit 5: HackInterview integration/e2e only when needed
```
...
### Slice 4 - HackInterview Legacy Context
...
```bash
pytest memory_platform/tests/e2e/test_legacy_hackinterview_context.py
...
ADR-0005: HackInterview compatibility routes
...
delete, profile isolation, or source-of-truth lifecycle for HackInterview.
...
- HackInterview legacy default maps to `external_ref=default`;
...
- HackInterview `session_id` maps to `external_ref`;
...
profile_id
thread_id?
source_app: hackinterview | api | sdk | manual
...
- default is `active_context` for HackInterview compatibility;
...
| `local_only` | HackInterview fallback only | local fallback | yes, local | no server writes |

Core Lite implementation:

```text
server supports disabled/manual_only/suggestions/active_context
HackInterview bridge still supports shadow_ingest/shadow_retrieve/local_only
...
| `GetSessionStatusUseCase` | thread/projection states | HackInterview status |
...
external refs
  HackInterview session_id, event_id, profile_id
```
...
### HackInterview Compatibility Route Example

```python
# packages/memory_server/memory_server/api/legacy_hackinterview.py
...
await client.remember_fact(
    space_id="space_hackinterview",
    profile_id="profile_default",
    text="HackInterview uses Qdrant for document RAG.",
    kind="architecture_decision",
    source_refs=[
        {
...
Keep legacy HackInterview routes out of generic SDK unless explicitly needed.
```
...
- HackInterview prompt path must call `/v1/context` or the legacy compatibility context route, never history/debug routes.
...
test_hackinterview_context_never_calls_history_routes
```
...
- suggestion review routes are not required for HackInterview active context first milestone, but ports/entities should allow them.
...
### HackInterview Anti-Corruption Layer

Legacy HackInterview DTOs are an adapter boundary. They must not become domain language.
...
```python
class HackInterviewMemoryMapper:
...
- no HackInterview env var is read inside `memory_core`;
...
- legacy HackInterview routes can map these errors to the old envelope.
...
- legacy HackInterview routes can downgrade errors to fallback-safe legacy responses;
...
curl -X PATCH http://127.0.0.1:7788/v1/facts/fact_123 \
  ...
  -d '{
    "expected_version": 1,
    "text": "HackInterview uses Qdrant for document RAG and Graphiti for temporal graph recall.",
...
- HackInterview compatibility routes are tested separately from public `/v1`.

Client compatibility promise for Core Lite:

```text
Within /v1, do not rename fields used by SDK or HackInterview legacy adapter.
...
- legacy HackInterview routes are not changed by public `/v1` evolution;
...
```text
service token -> principal type service -> space hackinterview -> profile default -> read/write/delete
```
...
- legacy HackInterview profile/session refs are never trusted as authorization;
...
canary
  HackInterview shadow ingest/retrieve
...
- no dependency may force HackInterview to import provider SDKs;
...
7. Seed default `hackinterview/default` only through a seed command, not migration.
...
HackInterview can produce long-running transcript streams. Core Lite should ingest them as episodes, not immediate active facts.
...
```bash
python -m memory_server.admin reindex-qdrant --space hackinterview --profile default
python -m memory_server.admin reindex-graphiti --space hackinterview --profile default
python -m memory_server.admin repair-projections --space hackinterview --dry-run
...
- `active_context` is the only mode that inserts remote memory into HackInterview prompt;
...
legacy context timeout budget in HackInterview: <= current bridge timeout
```
...
postgres timeout -> memory context fails closed and HackInterview falls back locally
```
...
no raw provider SDK import in HackInterview
```
...
legacy HackInterview route tests
```
...
HackInterview fallback canary passes
```
...
### Phase 2 - HackInterview Compatibility Gateway
...
| HackInterview field | Memory Platform field |
|---|---|
| `session_id` | `MemoryThread.external_ref` |
| `profile_id` | `MemoryProfile.external_ref` |
| current user/app | `MemorySpace.slug=hackinterview` |
...
| Phase 2 compatibility | HackInterview can call server | `INTERVIEW_MEMORY_FORCE_LOCAL_ONLY=true` | legacy route E2E |
...
```text
No phase can change HackInterview active prompt context until it has a shadow run, E2E canary, and kill switch.
```
...
no provider SDK import in HackInterview
noop adapters work
...
```text
HackInterview fallback canary pass
...
- legacy HackInterview memory auth override can rotate independently from main backend auth;
...
```bash
python -m memory_server.admin token create --space hackinterview --description local-dev
python -m memory_server.admin token revoke --token-id tok_123
python -m memory_server.admin token list --space hackinterview
```
...
### HackInterview Memory Server Timeout
...
| memory_server down | interview blocked | desktop local fallback continues | HackInterview canary |
...
| new server against old HackInterview | legacy break | legacy compatibility routes remain stable | e2e test |
...
| SDK timeout during context | prompt blocked | SDK raises retryable timeout and HackInterview falls back | SDK/e2e test |
...
restricted excluded from HackInterview context unless profile policy allows
```
...
memory_platform/tests/e2e/test_legacy_hackinterview_context.py
...
|---|---|---|---|
| Postgres unavailable on context | stop DB or fake repo raises | API error, HackInterview fallback | e2e |
...
- HackInterview canary starts in shadow mode first.
...
```bash
python -m memory_server.admin export-profile --space hackinterview --profile default --out profile-export.json
python -m memory_server.admin import-profile --space hackinterview --profile default --file profile-export.json --dry-run
```
...
```text
HackInterview falls back locally
...
```bash
python -m memory_server.admin repair-projections --space hackinterview --profile default --dry-run
python -m memory_server.admin reindex-qdrant --space hackinterview --profile default
```
...
python -m memory_server.admin reindex-graphiti --space hackinterview --profile default --dry-run
```
...
- HackInterview can point to memory_server and pass current memory canaries;
...
unit tests pass
no HackInterview behavior changes
```
...
Does HackInterview fallback still work?
...
no Qdrant
no Graphiti
no HackInterview prompt changes
```
...
### PR 2 - HackInterview Compatibility
...
3. Resolve default `hackinterview/default` scope.
...
HackInterview compatibility routes/anti-corruption: 1200-2400
