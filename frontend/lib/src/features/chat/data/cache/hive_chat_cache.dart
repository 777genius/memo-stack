...
import 'package:frontend/src/features/chat/domain/entities/chat_session_mapper.dart';
...
import 'package:frontend/src/features/chat/data/cache/chat_message_cache_mapper.dart';
...
class HiveChatCache implements ChatCache {
  ...
  Future<List<ChatMessage>> loadMessages(String chatId) async {
    ...
    for (final key in b.keys) {
      if (key is String && key.startsWith(prefix)) {
        ...
        if (v is Map) {
          try {
            out.add(ChatMessageCacheMapper.fromCacheMap(v));
          } catch (_) {}
        }
      }
    }
  ...
  Future<void> saveMessage(String chatId, ChatMessage message) async {
    ...
    final map = message.copyWith(chatId: chatId).toCacheMap(
          screenshotDir: dir,
        );
  ...
  Future<void> removeMessages(String chatId) async {
    ...
    for (final key in b.keys) {
      if (key is String && key.startsWith(prefix)) {
        ...
        if (v is Map) {
          try {
            ChatMessageCacheMapper.deleteScreenshot(v);
          } catch (_) {}
        }
