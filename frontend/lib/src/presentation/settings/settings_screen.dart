...
import 'package:frontend/src/app/services/api_key_validator.dart';
import 'package:frontend/src/app/services/secure_storage_service.dart';
import 'package:frontend/src/presentation/settings/widgets/api_key_field.dart';
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  late TextEditingController _spaceSlugController;
  late TextEditingController _memoryScopeRefController;
  ...
  void initState() {
    ...
    _spaceSlugController = TextEditingController(text: config.spaceSlug);
    _memoryScopeRefController =
        TextEditingController(text: config.memoryScopeExternalRef);
  ...
  Future<void> _loadSavedKeys() async {
    ...
    try {
      ...
      final savedHost = await _storage.getBackendHost();
      final savedPort = await _storage.getBackendPort();
      final savedToken = await _storage.getServiceToken();
      final savedSpace = await _storage.getSpaceSlug();
      final savedMemoryScope = await _storage.getMemoryScopeExternalRef();
      if (!mounted) return;
      setState(() {
        ...
        if (savedSpace != null) {
          _spaceSlugController.text = savedSpace;
        }
        if (savedMemoryScope != null) {
          _memoryScopeRefController.text = savedMemoryScope;
        }
      });
    } finally {
      if (mounted) {
  ...
  void dispose() {
    ...
    _spaceSlugController.dispose();
    _memoryScopeRefController.dispose();
  ...
  Future<void> _saveSettings() async {
    ...
    try {
      ...
      final spaceSlug = _spaceSlugController.text.trim().isEmpty
          ? 'default'
          : _spaceSlugController.text.trim();
      final memoryScopeRef = _memoryScopeRefController.text.trim().isEmpty
          ? 'default'
          : _memoryScopeRefController.text.trim();
      await _storage.saveBackendHost(_hostController.text.trim());
      await _storage.saveBackendPort(_portController.text.trim());
      await _storage.saveServiceToken(token);
      await _storage.saveSpaceSlug(spaceSlug);
      await _storage.saveMemoryScopeExternalRef(memoryScopeRef);
      ...
      config.update(
        ...
        token: token,
        spaceSlug: spaceSlug,
        memoryScopeExternalRef: memoryScopeRef,
    ...
    return Column(
      ...
      children: [
        ...
        if (_showAdvanced) ...[
          ...
          const SizedBox(height: 16),
          TextFormField(
            controller: _spaceSlugController,
            style: TextStyle(color: colorScheme.onSurface),
            decoration: InputDecoration(
              labelText: 'Space Slug',
              labelStyle: TextStyle(color: colorScheme.onSurfaceVariant),
              border: OutlineInputBorder(
                borderSide: BorderSide(color: colorScheme.outline),
              ),
          ...
          const SizedBox(height: 16),
          TextFormField(
            controller: _memoryScopeRefController,
            style: TextStyle(color: colorScheme.onSurface),
            decoration: InputDecoration(
              labelText: 'Memory scope ref',
              labelStyle: TextStyle(color: colorScheme.onSurfaceVariant),
              border: OutlineInputBorder(
                borderSide: BorderSide(color: colorScheme.outline),
              ),
