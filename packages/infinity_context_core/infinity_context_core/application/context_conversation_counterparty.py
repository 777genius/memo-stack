...
_CONVERSATION_RECENCY_QUERY_RE = re.compile(
    ...
    r"\b(?:говорил\w*|разговаривал\w*|общал\w*|переписывал\w*|"
    r"созванивал\w*|обсуждал\w*|написал\w*|ответил\w*|"
    r"сказал\w*|рассказал\w*|сообщил\w*|скинул\w*|"
    r"прислал\w*|отправил\w*|"
    r"разговор\w*|созвон\w*|встреч\w*|"
...
_CONVERSATION_EVENT_TEXT_RE = re.compile(
    ...
    r"\b[А-ЯЁA-Z][\wА-Яа-яЁё._-]{1,}\s+"
    r"(?:написал\w*|ответил\w*|сказал\w*|рассказал\w*|"
    r"сообщил\w*|скинул\w*|прислал\w*|отправил\w*)\b",
    re.IGNORECASE | re.DOTALL,
)
