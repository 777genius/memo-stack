"""HTTP adapter from MCP tools to the Infinity Context API."""
        ...
        async with httpx.AsyncClient(
            ...
            try:
                ...
                raise MemoryGatewayError(
                    status_code=0,
                    code="infinity_context_mcp.gateway.connect_timeout",
                    message="Infinity Context connection timed out",
                    retryable=True,
...
def _to_error(response: httpx.Response) -> MemoryGatewayError:
    ...
    retryable = bool(
        error.get(
            ...
            or code == "infinity_context_mcp.degraded.backpressure",
        )
    )
