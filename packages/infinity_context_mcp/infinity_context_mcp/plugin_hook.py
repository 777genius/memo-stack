"""Lifecycle hook runner for Infinity Context agent plugins.
...
class MemoryPluginHookApp:
        ...
        try:
            ...
            if event.name in self._settings.context_events:
                if contains_sensitive_value(query):
                    stderr_parts.append(
                        "infinity-context-plugin-hook: skipped memory lookup because hook input "
                        "looks sensitive\n"
                    )
                else:
            ...
            if (
                ...
                if contains_sensitive_value(source_text):
                    stderr_parts.append(
                        "infinity-context-plugin-hook: episode capture skipped because input "
                        "looks sensitive\n"
                    )
                else:
    ...
    def _build_context(self, event: HookEvent, query: str) -> tuple[str, str]:
        ...
        if not isinstance(data, dict):
            return "", "infinity-context-plugin-hook: /v1/context response missing data object\n"
    ...
    def _capture(self, event: HookEvent, text: str) -> str:
        ...
        if transcript_tail is not None:
            ...
            return "infinity-context-plugin-hook: capture skipped: transcript tail unavailable\n"
        ...
        if raw_capture_text_looks_sensitive or contains_sensitive_value(minimized):
            return "infinity-context-plugin-hook: capture skipped because input looks sensitive\n"
        try:
            ...
            return f"infinity-context-plugin-hook: capture skipped: {safe_message(str(exc))}\n"
        if self._settings.verbose:
            return "infinity-context-plugin-hook: capture accepted\n"
        return ""

    def _error_result(self, event: HookEvent, exc: Exception) -> HookResult:
        message = f"infinity-context-plugin-hook: {safe_message(str(exc))}\n"
...
def _render_memory_context(event_name: str, rendered_text: str) -> str:
    ...
    return (
        f'<memory_context source="infinity-context" event="{safe_event}">\n'
