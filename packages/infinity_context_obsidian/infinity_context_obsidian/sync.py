...
from infinity_context_obsidian.frontmatter import FrontmatterError, split_frontmatter
...
from infinity_context_obsidian.ports import MemoryGatewayPort, SyncStateStorePort, VaultPort
    ...
    def _import_path(self, *, path: PurePosixPath, apply: bool) -> ImportChange:
        ...
        if note.fact_id in self._seen_fact_ids:
            return ImportChange(
                ...
                message="Duplicate infinity_context_id in vault",
            )
        ...
        rendered = render_fact_note(
            updated_fact,
            space_slug=str(note.frontmatter.get("infinity_context_space_slug") or ""),
            memory_scope_external_ref=str(
                note.frontmatter.get("infinity_context_memory_scope_external_ref") or ""
            ),
        )
    ...
    def _import_path(
        ...
        *,
        ...
        self.state.save(
            SyncStateRecord(
                path=path,
                infinity_context_id=fingerprint,
                kind=NoteKind.INBOX,
                version=1,
