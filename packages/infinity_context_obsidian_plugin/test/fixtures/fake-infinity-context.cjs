...
const vault = process.env.INFINITY_CONTEXT_OBSIDIAN_VAULT || process.cwd();

const memoDir = path.join(vault, ".infinity-context");
fs.mkdirSync(memoDir, { recursive: true });

delayFromEnv("INFINITY_CONTEXT_FAKE_LOCAL_DELAY_MS");
if (process.env.INFINITY_CONTEXT_FAKE_LOCAL_FAIL_COMMAND === command) {
  fail(process.env.INFINITY_CONTEXT_FAKE_LOCAL_FAIL_MESSAGE || `Forced fake local stack failure: ${command}`);
}

if (command === "status") {
  const ready = process.env.INFINITY_CONTEXT_FAKE_LOCAL_STATUS_READY !== "false";
  emit({
    ok: true,
...
if (command === "init") {
  emit({
    ok: true,
    home: path.join(vault, ".infinity-context-home"),
...
fail(`Unknown fake infinity-context command: ${args.join(" ")}`);
