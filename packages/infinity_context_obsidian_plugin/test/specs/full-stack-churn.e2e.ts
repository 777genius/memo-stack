...
describe("Infinity Context generated note churn E2E", function () {
  let server: ChildProcess | undefined;
  let tempDir = "";
  let baseUrl = "";

  beforeEach(async function () {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "infinity-context-wdio-churn-"));
  ...
  it("recovers deleted clean notes while applying edits and exporting new backend facts", async function () {
    ...
    assert.match(recoveredFirst, /infinity_context_version: 1/);
    assert.match(recoveredSecond, /Obsidian WDIO churn initial fact 3/);
    assert.match(recoveredSecond, /infinity_context_version: 1/);
    ...
    assert.match(editedMarkdown, /infinity_context_version: 2/);
    ...
    assert.equal(conflictFiles(vaultPath).length, 0);
    assert.equal(factFiles(vaultPath).length, initialFacts.length + lateFacts.length);
    assert.match(fs.readFileSync(factFileForId(vaultPath, initialFacts[4].id), "utf8"), /infinity_context_version: 2/);
  });
