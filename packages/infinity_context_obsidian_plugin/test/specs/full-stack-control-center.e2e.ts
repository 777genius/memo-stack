...
describe("Infinity Context Control Center E2E", function () {
  ...
  beforeEach(async function () {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "infinity-context-wdio-control-center-"));
    port = await freePort();
    baseUrl = `http://127.0.0.1:${port}`;
    dbPath = path.join(tempDir, "memory.db");
    server = startInfinityContextServer(dbPath, port);
    await waitForHealth(baseUrl);
  });

  afterEach(async function () {
    await stopInfinityContextServer(server);
  ...
  it("runs connect, preview, sync and navigation through Control Center buttons", async function () {
    ...
    await waitForPanelText("Infinity Context");
    ...
    assert.match(panel.text, /Generated\s+Infinity Context\/spaces\/wdio-control-center\/memory_scopes\/default\/generated\/facts/);
    ...
    assert.equal(await activeFilePath(), "Infinity Context/README.md");
...
async function stopInfinityContextServer(server: ChildProcess | undefined): Promise<void> {
...
async function waitForServerDown(apiUrl: string): Promise<void> {
  ...
  }, "Infinity Context server did not stop");
}
  ...
  await browser.waitUntil(async () => {
    return await browser.execute((buttonLabel) => {
      const buttons = Array.from(
        document.querySelectorAll(".infinity-context-vault-actions button"),
