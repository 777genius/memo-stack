...
describe("Infinity Context friendly project names E2E", function () {
  let server: ChildProcess | undefined;
  let tempDir = "";
  let baseUrl = "";

  beforeEach(async function () {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "infinity-context-wdio-friendly-names-"));
  ...
  it("stores friendly project and memory_scope names in safe scoped vault folders", async function () {
    ...
    assert.match(fs.readFileSync(exportedFact, "utf8"), /infinity_context_space_slug: Client Alpha \/ Research 2026/);
    assert.match(fs.readFileSync(exportedFact, "utf8"), /infinity_context_memory_scope_external_ref: Default User\+Agent/);
  ...
  it("configures a friendly scoped vault through the Obsidian settings UI", async function () {
    ...
    assert.match(readVaultFile(vaultPath, path.join(".obsidian", "plugins", "infinity-context", "data.json")), /Client Alpha/);
  ...
  it("preserves pending old-memory_scope edits and inbox notes across memory_scope switches", async function () {
    ...
    assert.match(fs.readFileSync(onlyFactFile(vaultPath, primaryScope), "utf8"), /infinity_context_version: 2/);
...
async function waitForPluginScope(scope: Scope): Promise<void> {
  await browser.waitUntil(
    ...
    {
      timeout: 20000,
      timeoutMsg: "Infinity Context plugin did not reload friendly scope settings",
    },
  );
}
...
async function waitForConnectedLayout(scope: Scope): Promise<void> {
  await browser.waitUntil(
    ...
    {
      timeout: 20000,
      timeoutMsg: "Infinity Context plugin did not observe the connected friendly layout",
    },
  );
}
...
async function waitForSettingsFile(vaultPath: string, marker: string = rawSpaceSlug): Promise<void> {
  ...
  await waitUntil(
    ...
    "Infinity Context settings UI did not persist data.json",
  );
}
