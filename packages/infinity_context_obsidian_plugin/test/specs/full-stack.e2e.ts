...
describe("Infinity Context full Obsidian E2E", function () {
  let server: ChildProcess | undefined;
  let tempDir = "";
  let baseUrl = "";

  beforeEach(async function () {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "infinity-context-wdio-full-"));
  ...
  it("syncs through real Obsidian plugin, connector CLI, HTTP API and vault files", async function () {
    ...
    await waitForVaultFile(vaultPath, "Infinity Context/README.md");
    assert.match(readVaultFile(vaultPath, "Infinity Context/README.md"), /Infinity Context/);
  ...
  it("keeps readonly managed note edits out of the backend and visible as conflicts", async function () {
    ...
    const vaultPath = await resetVaultAndConfigure(baseUrl);
    const exportedFact = await connectAndExportFact(vaultPath);
    replaceFrontmatterValue(exportedFact, "infinity_context_sync_mode", "readonly");
  ...
  it("turns duplicate managed fact IDs in the vault into visible conflicts", async function () {
    ...
    const conflict = fs.readFileSync(conflicts[0], "utf8");
    assert.match(conflict, /Infinity Context Sync Conflict/);
    assert.match(conflict, /Duplicate infinity_context_id in vault/);
  ...
  it("recovers from stale conflicts after the dirty local note is removed", async function () {
    ...
    assert.match(recovered, /infinity_context_version: 2/);
  ...
  it("turns corrupt managed notes into visible conflicts without overwriting them", async function () {
    ...
    const vaultPath = await resetVaultAndConfigure(baseUrl);
    const exportedFact = await connectAndExportFact(vaultPath);
    fs.writeFileSync(exportedFact, "not a infinity context fact note", "utf8");
    ...
    assert.equal(calls.at(-1)?.command, "sync");
    assert.equal(calls.at(-1)?.status, 1);
    assert.equal(fs.readFileSync(exportedFact, "utf8"), "not a infinity context fact note");
    assert.equal(
      (await getFact(baseUrl, fact.id)).text,
...
async function waitForUnhealthy(apiUrl: string): Promise<void> {
  ...
  }, "Infinity Context server stayed healthy");
}
