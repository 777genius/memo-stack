...
const fakeLocalCliPath = path.resolve("test/fixtures/fake-infinity-context.cjs");
...
const fakeEnvKeys = [
  "INFINITY_CONTEXT_FAKE_OBSIDIAN_DELAY_MS",
  "INFINITY_CONTEXT_FAKE_OBSIDIAN_FAIL_COMMAND",
  "INFINITY_CONTEXT_FAKE_OBSIDIAN_FAIL_MESSAGE",
  "INFINITY_CONTEXT_FAKE_LOCAL_DELAY_MS",
  "INFINITY_CONTEXT_FAKE_LOCAL_FAIL_COMMAND",
  "INFINITY_CONTEXT_FAKE_LOCAL_FAIL_MESSAGE",
  "INFINITY_CONTEXT_FAKE_LOCAL_STATUS_READY",
];

describe("Infinity Context Obsidian plugin", function () {
  ...
  it("registers commands and delegates sync actions to the connector CLI", async function () {
    ...
    await browser.executeObsidianCommand("infinity-context:local-stack-init");
    await waitForLocalStackCalls(vaultPath, 3);
    await waitForPluginIdle();
    await browser.executeObsidianCommand("infinity-context:local-stack-status");
    await waitForLocalStackCalls(vaultPath, 4);
    await waitForPluginIdle();
    await browser.executeObsidianCommand("infinity-context:start-local-stack-lite");
    ...
    await waitForPluginIdle();
    const readme = readVaultFile(vaultPath, "Infinity Context/README.md");
    assert.match(readme, /Connected by plugin E2E/);

    await browser.executeObsidianCommand("infinity-context:run-doctor");
    ...
    await browser.executeObsidianCommand("infinity-context:open-infinity-context-readme");
    const activeFilePath = await browser.executeObsidian(({ app }) => {
      return app.workspace.getActiveFile()?.path ?? "";
    });
    assert.equal(activeFilePath, "Infinity Context/README.md");
    ...
    assert.equal(
      activeConflictPath,
      ["Infinity Context", "spaces", spaceSlug, "memory_scopes", memoryScopeExternalRef, "conflicts", "README.md"].join("/"),
    );
  ...
  it("surfaces connector failures and clears busy state", async function () {
    const vaultPath = await resetVaultAndConfigure();
    await setFakeEnv({ INFINITY_CONTEXT_FAKE_OBSIDIAN_FAIL_COMMAND: "preview" });
  ...
  it("redacts secrets from connector and local stack command snapshots", async function () {
    ...
    await setFakeEnv({
      INFINITY_CONTEXT_FAKE_OBSIDIAN_FAIL_COMMAND: "preview",
      INFINITY_CONTEXT_FAKE_OBSIDIAN_FAIL_MESSAGE: `Authorization: Bearer ${connectorSecret} token=${keyValueSecret}`,
    });
    await browser.executeObsidianCommand("infinity-context:preview-sync");
    await waitForCliCalls(vaultPath, 1);
    await waitForPluginIdle();
    ...
    await setFakeEnv({
      INFINITY_CONTEXT_FAKE_LOCAL_FAIL_COMMAND: "init",
      INFINITY_CONTEXT_FAKE_LOCAL_FAIL_MESSAGE: `password=hunter2-secret-value ${localSecret}`,
    });
    await browser.executeObsidianCommand("infinity-context:local-stack-init");
    await waitForLocalStackCalls(vaultPath, 1);
    await waitForPluginIdle();
  ...
  it("handles a missing connector binary without leaving the plugin busy", async function () {
    const vaultPath = await resetVaultAndConfigure({
      cliPath: path.join(vaultPathPlaceholder(), "missing-infinity-context-obsidian"),
    });
  ...
  it("clears busy state when a connector command times out", async function () {
    ...
    await setFakeEnv({ INFINITY_CONTEXT_FAKE_OBSIDIAN_DELAY_MS: "5000" });
  ...
  it("stops prepare before vault writes when local init fails", async function () {
    const vaultPath = await resetVaultAndConfigure();
    await setFakeEnv({ INFINITY_CONTEXT_FAKE_LOCAL_FAIL_COMMAND: "init" });
  ...
  it("prepares the vault but skips preview when the local API is not ready", async function () {
    const vaultPath = await resetVaultAndConfigure();
    await setFakeEnv({ INFINITY_CONTEXT_FAKE_LOCAL_STATUS_READY: "false" });
    ...
    assert.equal(
      activeInboxPath,
      ["Infinity Context", "spaces", spaceSlug, "memory_scopes", memoryScopeExternalRef, "inbox", "README.md"].join("/"),
    );
  });
...
async function panelState(): Promise<{
  ...
  return await browser.execute(() => {
    const panel = document.querySelector(".infinity-context-panel");
    if (!panel) {
      return { text: "", buttons: {} };
    }
  ...
  await browser.waitUntil(async () => {
    return await browser.execute((buttonLabel) => {
      const buttons = Array.from(
        document.querySelectorAll(".infinity-context-panel button"),
...
function readLocalStackCalls(vaultPath: string): Array<{
  ...
  const logPath = path.join(vaultPath, ".infinity-context/local-stack-calls.jsonl");
  if (!fs.existsSync(logPath)) {
    return [];
  }
  return fs
