...
def _quality_golden_cases(
    *,
    ...
    return (
        ...
        EvalCase(
            case_id="longmemeval_knowledge_update_current_truth",
            category="stale_update",
            space_id=space_id,
            memory_scope_ids=(alpha_memory_scope_id,),
            query=(
                "LongMemEval knowledge update old pgvector provider current "
                "Qdrant Graphiti truth"
            ),
            must_include=("QUALITY_FACT_PROVIDER_CURRENT",),
            must_not_include=("QUALITY_FACT_PROVIDER_OLD",),
            max_facts=5,
            max_chunks=0,
            required_diagnostics=(("items_used", "gte", 1),),
        ),
        EvalCase(
        ...
        EvalCase(
            case_id="mixed_language_wrong_project_returns_no_context",
            category="no_candidate",
            space_id=space_id,
            memory_scope_ids=(alpha_memory_scope_id,),
            query="Алекс Project Apollo billing час назад",
            must_not_include=(
                "QUALITY_FACT_ALEX_ATLAS_RECENT",
                "QUALITY_FACT_RU_ALEX_ATLAS",
                "QUALITY_FACT_MARIA_ATLAS_WEEK",
                "QUALITY_FACT_RELATIVE_TIME_CURRENT",
            ...
            required_diagnostics=(
                ("items_used", "eq", 0),
                (
                    "requirement_guard_status",
                    "eq",
                    "dropped_missing_project_anchor",
                ),
                ("requirement_guard_items_dropped", "gte", 1),
                (
        ...
        EvalCase(
            case_id="longmemeval_multilingual_entity_abstention",
            category="no_candidate",
            space_id=space_id,
            memory_scope_ids=(alpha_memory_scope_id,),
            query="Никита Project Mercury billing вчера",
            must_not_include=(
                "QUALITY_FACT_ALEX_ATLAS_RECENT",
                "QUALITY_FACT_RU_ALEX_ATLAS",
                "QUALITY_FACT_MARIA_ATLAS_WEEK",
        ...
        EvalCase(
            case_id="longmemeval_source_attribution_project_anchor",
            category="anchor_context",
            space_id=space_id,
            memory_scope_ids=(alpha_memory_scope_id,),
            query="Project Atlas source attribution citation evidence OCR screenshot anchor",
            must_include=("project: Project Atlas",),
            max_facts=0,
            max_chunks=0,
            required_diagnostics=(
        ...
        EvalCase(
            ...
            required_item_matches=(
                (
                    ...
                    (
                        "diagnostics.evidence_profile.schema_version",
                        "eq",
                        "context-item-evidence-profile-v1",
                    ),
                    ("diagnostics.evidence_profile.has_citations", "eq", True),
                    ("diagnostics.evidence_profile.has_precise_location", "eq", True),
                    ("diagnostics.evidence_profile.has_multimodal_location", "eq", True),
                    (
                        "diagnostics.evidence_profile.source_refs_with_bbox_count",
                        "gte",
                        1,
                    ),
                    (
                        "diagnostics.evidence_profile.source_refs_with_time_range_count",
                        "gte",
                        1,
                    ),
...
def _long_memory_golden_cases(
    *,
    ...
    return (
        ...
        EvalCase(
            case_id="long_lme_knowledge_update_old_query_current_truth",
            category="temporal_update",
            space_id=space_id,
            memory_scope_ids=(alpha_memory_scope_id,),
            query="old pgvector provider disabled graph search documents current truth",
            must_include=("LONGMEM_PROVIDER_CURRENT",),
            must_not_include=("LONGMEM_PROVIDER_OLD",),
            max_facts=10,
            max_chunks=0,
        ...
        EvalCase(
            case_id="long_lme_multilingual_preference_recall",
            category="preference_synthesis",
            space_id=space_id,
            memory_scope_ids=(alpha_memory_scope_id,),
            query="короткие Russian summaries concrete next actions review gated deletes",
            must_include=("LONGMEM_PREF_FORMAT", "LONGMEM_CONSTRAINT_REVIEW"),
            max_facts=5,
            max_chunks=0,
        ),
        ...
        EvalCase(
            case_id="long_lme_abstention_unknown_multilingual",
            category="no_candidate",
            space_id=space_id,
            memory_scope_ids=(alpha_memory_scope_id,),
            query="Никита Mercury quantum aquarium warranty",
            must_not_include=(
                "LONGMEM_SESSION_KICKOFF",
                "LONGMEM_PREF_FORMAT",
                "LONGMEM_PROVIDER_CURRENT",
                "LONGMEM_DOC_PROJECT_SCOPE",
                "LONGMEM_DECISION_GRAPHITI",
            ),
            max_facts=1,
            max_chunks=1,
            consistency_mode="canonical_only",
