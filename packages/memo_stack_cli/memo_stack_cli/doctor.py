"""Doctor checks for local Memo Stack."""
...
from memo_stack_cli.config import DEFAULT_SERVICE_TOKEN, MemoStackCliConfig
from memo_stack_cli.runtime import docker_available, docker_compose_available
...
def run_doctor(config: MemoStackCliConfig, *, timeout: float = 3.0) -> list[DoctorCheck]:
    checks = [
        DoctorCheck(
            name="repo_root",
...
def doctor_payload(config: MemoStackCliConfig, checks: list[DoctorCheck]) -> dict[str, Any]:
    return {
...
def _api_checks(config: MemoStackCliConfig, *, timeout: float) -> list[DoctorCheck]:
