...
def build_mcp_config(
    *,
    ...
    if agent in {"codex", "cursor"}:
        return {"memo-stack": {"command": command, "env": env}}
    return {"mcpServers": {"memo-stack": {"command": command, "env": env}}}
...
def _mcp_command(repo_dir: Path) -> str:
    plugin_command = repo_dir / "plugins" / "memo-stack-agent-plugin" / "bin" / "memo-stack-mcp"
    if plugin_command.exists():
        return str(plugin_command)
    return "memo-stack-mcp"
