"""MCP-facing helpers for local Memo Stack runtime setup."""
...
import httpx
from memo_stack_cli.config import (
    DEFAULT_API_URL,
    DEFAULT_HOME,
...
class LocalRuntimeMcpService:
    ...
    def _ensure_enabled(self) -> None:
        if not self._settings.local_runtime_enabled:
            raise MemoryGatewayError(
                status_code=403,
                code="memo_stack_mcp.local_runtime.disabled",
    ...
    def _ensure_start_enabled(self) -> None:
        if not self._settings.local_runtime_start_enabled:
            raise MemoryGatewayError(
                status_code=403,
                code="memo_stack_mcp.local_runtime.start_disabled",
    ...
    async def _guard(self, action) -> dict[str, Any]:
        try:
            ...
            return self._error(
                code="memo_stack_mcp.local_runtime.error",
                message=str(exc),
                status_code=500,
                retryable=True,
            )
    ...
    def _error(
        ...
        *,
        ...
        public_code = (
            code
            if code.startswith("memo_stack_mcp.local_runtime.")
