...
from memo_stack_obsidian.doctor import DoctorVaultUseCase
...
class ObsidianMcpService:
    ...
    def _context(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        state = SqliteSyncStateStore(vault_path / ".memo-stack" / "obsidian-sync.sqlite3")
    ...
    def _require_vault_path(self, request: dict[str, Any]) -> Path:
        ...
        if not vault_path:
            raise MemoryGatewayError(
                status_code=400,
                code="memo_stack_mcp.obsidian.missing_vault",
    ...
    def _ensure_obsidian_enabled(self) -> None:
        if not self._settings.obsidian_enabled:
            raise MemoryGatewayError(
                status_code=403,
                code="memo_stack_mcp.obsidian.disabled",
    ...
    def _ensure_sync_enabled(self) -> None:
        if not self._settings.obsidian_sync_enabled:
            raise MemoryGatewayError(
                status_code=403,
                code="memo_stack_mcp.obsidian.sync_disabled",
    ...
    async def _guard(self, action) -> dict[str, Any]:
        try:
            return await action()
        except (MemoStackError, httpx.HTTPError, OSError, ValueError) as exc:
            return self._error(
                code="memo_stack_mcp.obsidian.error",
                message=str(exc),
                status_code=500,
                retryable=True,
            )
    ...
    def _error(
        ...
        *,
        ...
        public_code = (
            code
            if code.startswith("memo_stack_mcp.obsidian.")
