...
import {
  ...
  type MemoStackPathPlan,
  type MemoStackSnapshot,
  defaultLayoutPaths,
  errorMessage,
  exitCode,
  joinUrl,
...
import { MemoStackControlView } from "./panel";

interface MemoStackSettings {
  apiUrl: string;
  token: string;
  localCliPath: string;
  cliPath: string;
...
const DEFAULT_SETTINGS: MemoStackSettings = {
...
export default class MemoStackPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.registerView(
      VIEW_TYPE_MEMO_STACK,
      (leaf) => new MemoStackControlView(leaf, this, VIEW_TYPE_MEMO_STACK),
    );

    this.addRibbonIcon("database", "Memo Stack", () => {
    ...
    this.addCommand({
      id: "open-memo-stack-readme",
      name: "Open Memo Stack README",
      callback: async () => {
        await this.openMemoStackReadme();
      },
    });

    this.addCommand({
      id: "open-inbox",
      name: "Open Memo Stack inbox",
      callback: async () => {
    ...
    this.addCommand({
      id: "open-conflicts",
      name: "Open Memo Stack conflicts",
      callback: async () => {
        await this.openConflicts();
      },
    });

    this.addSettingTab(new MemoStackSettingTab(this.app, this));
  }

  async onunload() {
    this.app.workspace.detachLeavesOfType(VIEW_TYPE_MEMO_STACK);
  }
  ...
  async checkDaemonHealth() {
    try {
      ...
      new Notice("Memo Stack daemon is healthy.");
    } catch (error) {
      new Notice(`Memo Stack health check failed: ${errorMessage(error)}`, 10000);
    }
  }
  ...
  async startLocalStackLite() {
    ...
    if (cooldownSeconds > 0) {
      new Notice(`Memo Stack start was just requested. Try again in ${cooldownSeconds}s.`);
  ...
  async openControlCenter() {
    ...
    const leaves = workspace.getLeavesOfType(VIEW_TYPE_MEMO_STACK);
    const leaf = leaves[0] ?? workspace.getRightLeaf(false);
    if (!leaf) {
      new Notice("Could not open Memo Stack panel.");
      return;
    }
    await leaf.setViewState({ type: VIEW_TYPE_MEMO_STACK, active: true });
  ...
  async prepareVault() {
    if (this.busyLabel) {
      new Notice(`Memo Stack is already running: ${this.busyLabel}`);
      return;
    }
    try {
      this.layoutPaths();
      ...
      if (init.exitCode !== 0) {
        new Notice("Memo Stack prepare stopped: local config was not initialized.", 12000);
        return;
      }

      const connect = await this.runPreparedConnectorStep("connect");
      if (connect.exitCode !== 0) {
        new Notice("Memo Stack prepare stopped: vault was not connected.", 12000);
        return;
      }

      const status = await this.runPreparedLocalStep("status");
      if (status.exitCode !== 0 || !localStackApiReady(status)) {
        new Notice("Memo Stack prepared. Start lite, then run Preview.", 10000);
        return;
      }
      ...
      new Notice(
        preview.exitCode === 0
          ? "Memo Stack prepared and preview checked."
          : "Memo Stack prepared, but preview needs attention.",
        preview.exitCode === 0 ? 8000 : 12000,
      );
    } catch (error) {
      new Notice(`Memo Stack prepare failed: ${errorMessage(error)}`, 12000);
    } finally {
      this.busyLabel = "";
      this.refreshControlViews();
    }
  ...
  async openMemoStackReadme() {
    await this.openVaultFile(this.layoutPaths().readme, "Memo Stack README");
  }
  ...
  snapshot(): MemoStackSnapshot {
    let pathError = "";
    let paths: MemoStackPathPlan;
    try {
  ...
  private async runAndReport(command: ConnectorCommand) {
    ...
    try {
      ...
      if (result.exitCode !== 0) {
        console.warn("Memo Stack connector exited with non-zero status", result);
      }
    } catch (error) {
  ...
  private async runLocalStackAndReport(command: LocalStackCommand) {
    ...
    try {
      ...
      new Notice(message, result.exitCode === 0 ? 8000 : 12000);
      if (result.exitCode !== 0) {
        console.warn("Memo Stack local stack command exited with non-zero status", result);
      }
    } catch (error) {
      new Notice(`Memo Stack local ${command} failed: ${errorMessage(error)}`, 12000);
    } finally {
      this.busyLabel = "";
      this.refreshControlViews();
    }
  ...
  private async runPreparedConnectorStep(
    ...
    this.lastResult = result;
    if (result.exitCode !== 0) {
      console.warn("Memo Stack prepare connector step failed", command, result);
    }
    return result;
  }
  ...
  private async runPreparedLocalStep(
    ...
    this.lastStackResult = result;
    if (result.exitCode !== 0) {
      console.warn("Memo Stack prepare local step failed", command, result);
    }
    return result;
  }
  ...
  private async runConnector(command: ConnectorCommand): Promise<ConnectorRunResult> {
    if (!Platform.isDesktopApp) {
      throw new Error("The Memo Stack plugin currently requires desktop Obsidian.");
    }
  ...
  private localStackEnv(): NodeJS.ProcessEnv {
    ...
    if (vaultPath) {
      env.MEMO_STACK_OBSIDIAN_VAULT = vaultPath;
    }
    return env;
  }
  ...
  private layoutPaths(): MemoStackPathPlan {
  ...
  private refreshControlViews() {
    for (const leaf of this.app.workspace.getLeavesOfType(VIEW_TYPE_MEMO_STACK)) {
...
class MemoStackSettingTab extends PluginSettingTab {
  plugin: MemoStackPlugin;

  constructor(app: App, plugin: MemoStackPlugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  ...
  display(): void {
    ...
    containerEl.empty();
    containerEl.createEl("h2", { text: "Memo Stack" });

    if (!Platform.isDesktopApp) {
      containerEl.createEl("p", {
        text: "This plugin currently requires desktop Obsidian.",
        cls: "memo-stack-settings-warning",
      });
    }
