...
export interface MemoStackPanelPlugin {
  ...
  openMemoStackReadme(): Promise<void>;
  openInbox(): Promise<void>;
  openConflicts(): Promise<void>;
}

export class MemoStackControlView extends ItemView {
  private plugin: MemoStackPanelPlugin;
  private viewType: string;

  constructor(leaf: WorkspaceLeaf, plugin: MemoStackPanelPlugin, viewType: string) {
    super(leaf);
    this.plugin = plugin;
  ...
  render() {
    ...
    contentEl.empty();
    contentEl.addClass("memo-stack-panel");

    contentEl.createEl("h2", { text: "Memo Stack" });

    const status = contentEl.createDiv({ cls: "memo-stack-status-grid" });
    ...
    const pathList = contentEl.createDiv({ cls: "memo-stack-path-list" });
...
function renderLocalStackActions(
  ...
  snapshot: MemoStackSnapshot,
  plugin: MemoStackPanelPlugin,
) {
  container.createEl("h3", { text: "Local stack" });
  const actions = container.createDiv({ cls: "memo-stack-action-grid" });
...
function renderVaultActions(
  ...
  const actions = container.createDiv({ cls: "memo-stack-action-grid" });
  actions.addClass("memo-stack-vault-actions");
  ...
  actionButton(actions, "README", () => void plugin.openMemoStackReadme(), {
...
function renderLastVaultRun(
  ...
  const last = container.createDiv({ cls: "memo-stack-last-run" });
  last.createSpan({
    text: result.exitCode === 0 ? "Last run: ok" : "Last run: failed",
    cls: result.exitCode === 0 ? "memo-stack-ok" : "memo-stack-bad",
  });
...
function renderSetupChecklist(
  ...
  const list = container.createDiv({ cls: "memo-stack-setup-list" });
  const primary = list.createDiv({ cls: "memo-stack-setup-primary" });
  ...
  if (snapshot.busyLabel) {
    list.createDiv({ text: `Running: ${snapshot.busyLabel}`, cls: "memo-stack-busy" });
  }
}
...
function setupStep(
  ...
  const row = container.createDiv({ cls: "memo-stack-setup-row" });
  row.createSpan({ text: options.index, cls: "memo-stack-setup-index" });
  row.createSpan({ text: options.label, cls: "memo-stack-status-label" });
  row.createSpan({
    text: options.status,
    cls: options.ok ? "memo-stack-ok" : "memo-stack-muted",
  });
...
function stackStepStatus(snapshot: MemoStackSnapshot): string {
...
function stackStepOk(snapshot: MemoStackSnapshot): boolean {
  return (
