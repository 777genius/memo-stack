...
function startMacFocusGuard() {
  if (process.platform !== "darwin" || process.env.MEMO_STACK_OBSIDIAN_E2E_FOCUS_GUARD === "0") {
    return () => {};
  }

  const initialRefocusApp = appleScriptString(process.env.MEMO_STACK_OBSIDIAN_E2E_REFOCUS_APP || frontmostAppName());
  const script = [
  ...
  guard.on("error", (error) => {
    if (!stopped) {
      console.warn(`[memo-stack] Obsidian focus guard failed: ${error.message}`);
    }
  });
  guard.on("exit", (code, signal) => {
    if (!stopped) {
      ...
      console.warn(`[memo-stack] Obsidian focus guard stopped unexpectedly: ${reason}${detail ? `: ${detail}` : ""}`);
    }
  });

  console.log(
    [
      "[memo-stack] Obsidian focus guard active.",
      "Set MEMO_STACK_OBSIDIAN_E2E_REFOCUS_APP to pin focus to a specific app.",
      "Disable with MEMO_STACK_OBSIDIAN_E2E_FOCUS_GUARD=0.",
    ].join(" "),
  );
  return () => {
    stopped = true;
