...
describe("Memo Stack import toggle E2E", function () {
  let server: ChildProcess | undefined;
  let tempDir = "";
  let baseUrl = "";

  beforeEach(async function () {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "memo-stack-wdio-import-toggle-"));
...
async function waitForApplyImport(value: boolean): Promise<void> {
  await browser.waitUntil(async () => {
    ...
    timeout: 20000,
    timeoutMsg: `Memo Stack plugin did not apply import toggle ${value}`,
  });
}
...
async function waitForSettingsFile(vaultPath: string, expectedText: string): Promise<void> {
  ...
  await waitUntil(
    ...
    "Memo Stack settings UI did not persist import toggle",
  );
}
