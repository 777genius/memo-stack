...
describe("Memo Stack multi-scope bulk Obsidian E2E", function () {
  let server: ChildProcess | undefined;
  let tempDir = "";
  let baseUrl = "";

  beforeEach(async function () {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "memo-stack-wdio-scope-bulk-"));
  ...
  it("keeps a large vault isolated to the configured project and profile", async function () {
    ...
    const foreignGeneratedMarkdown = [
      ...
      "",
      "This pre-existing note belongs to another Memo Stack project.",
    ...
    assert.equal(factFiles(vaultPath).length, visibleFacts.length);
    assert.equal(conflictFiles(vaultPath).length, 0);
    assert.match(fs.readFileSync(factFileForId(vaultPath, visibleFacts[5].id), "utf8"), /memo_stack_version: 2/);
    assert.equal(
      readVaultFile(vaultPath, path.join(otherScopeRoot, "generated", "facts", "foreign-owned.md")),
      foreignGeneratedMarkdown,
    );
