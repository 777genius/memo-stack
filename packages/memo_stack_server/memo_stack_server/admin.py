...
from memo_stack_core.application import EnsureScopeCommand
from sqlalchemy import func, select, text
from sqlalchemy.ext.asyncio import AsyncSession

from memo_stack_server.admin_invariants import invariant_check
from memo_stack_server.admin_outbox import compact_done_outbox, replay_outbox
from memo_stack_server.admin_projection_repair import (
    reindex_graphiti,
    reindex_qdrant,
    repair_projections,
)
...
from memo_stack_server.memory_scope_transfer import export_memory_scope, import_memory_scope
    ...
    *,
    ...
    checks = [
        _gate_check(
            ...
            remediation="python -m memo_stack_server.doctor",
            doctor_status=str(doctor_result.get("status")),
        ),
        _gate_check(
            "default_scope",
            str(default_scope["status"]),
            remediation="python -m memo_stack_server.admin seed-defaults",
        ),
        _gate_check(
            "outbox_dead_count",
            "ok" if dead_outbox == 0 else "failed",
            remediation="python -m memo_stack_server.admin replay-outbox --status dead --limit 50",
            dead=dead_outbox,
        ...
        _gate_check(
            ...
            remediation="python -m memo_stack_server.admin check-invariants --include-projections",
