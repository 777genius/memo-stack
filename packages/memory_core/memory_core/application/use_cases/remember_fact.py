...
from memory_core.application.dto import FactResult, RememberFactCommand
from memory_core.application.normalize import scoped_idempotency_key
from memory_core.domain.entities import MemoryFact, MemoryFactId
...
def remember_fact_fingerprint(command: RememberFactCommand) -> str:
    ...
    raw = (
        ...
        f"{command.kind}:{command.text}:{command.classification}:{source_keys}"
    )
