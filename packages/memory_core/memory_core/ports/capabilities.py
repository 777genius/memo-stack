...
These contracts describe what Memory Core can ask from an engine adapter. They
do not describe provider SDKs. Canonical lifecycle stays in Memory Core and
...
class RagRecallPort(EngineHealthPort, Protocol):
    async def recall(self, query: CapabilityRecallQuery) -> CapabilityRecallResult:
        """Return RAG candidates that Memory Core may hydrate and rank."""
