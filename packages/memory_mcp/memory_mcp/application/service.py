...
from memory_mcp.application.policy import MemoryPolicyService
...
class MemoryToolService:
        ...
        async def action() -> dict[str, Any]:
            ...
            if not candidates:
                raise MemoryGatewayError(
                    status_code=400,
                    code="memory_mcp.validation.input_too_large",
            ...
            for index, raw_candidate in enumerate(candidates):
                ...
                try:
                    ...
                    bucket = (
                        "conflicts"
                        if decision_code.startswith("memory_mcp.conflict.")
    ...
    async def _proposal_remember(
        ...
        if policy.direct_allowed and not self._has_direct_write_evidence(
            ...
            payload = await self._gateway.create_suggestion(
                ...
                trust_level="medium",
                safe_reason="memory_mcp.policy.evidence_required",
            )
            return {
                **self._candidate_result(
                    candidate_index,
    ...
    def _source_ref(
        ...
        *,
        ...
        try:
            ...
            raise MemoryGatewayError(
                status_code=400,
                code="memory_mcp.validation.invalid_source_ref",
                message=str(exc),
                retryable=False,
            ) from exc
    ...
    def _ensure_writes_allowed(self) -> None:
        if not self._settings.writes_enabled:
            raise MemoryGatewayError(
                status_code=403,
                code="memory_mcp.policy.write_mode_off",
                message="Memory MCP writes are disabled by local policy",
                retryable=False,
            )
    ...
    def _ensure_deletes_allowed(self) -> None:
        if not self._settings.deletes_enabled:
            raise MemoryGatewayError(
                status_code=403,
                code="memory_mcp.policy.delete_mode_off",
                message="Memory MCP deletes are disabled by local policy",
                retryable=False,
            )
        ...
        try:
            ...
            response = McpToolResponse(
                ...
                diagnostics=McpDiagnostics(
                    ...
                    degraded=code.startswith(("memory_mcp.gateway.", "memory_mcp.degraded.")),
                ),
            )
