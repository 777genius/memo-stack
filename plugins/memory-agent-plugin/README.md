...
Codex and Claude package lanes also include `hooks/hooks.json`, backed by the
repo-local wrapper at `bin/memory-plugin-hook`. Hooks are intentionally separate
from MCP:
...
- Hook failures are fail-open by default, so an unavailable Memory Server does
  not block the agent.
...
The generated cross-agent hook file currently wires `UserPromptSubmit` and
`Stop`, because that is the smallest common safe subset for Codex and Claude.
`SessionStart` is supported by the runner for agents or installs that wire it
explicitly.
...
`git diff --check` and a repository secret scan. It also runs `memory-down` on
exit so the Docker smoke stack is not left behind.
`memory-prod-confidence-strict` is the fully strict paid/local-auth gate. It
...
`memory-prod-confidence-strict-preflight` first, so missing key/auth fails before
the paid full-provider canary. `memory-prod-confidence-full` is an alias for the
same gate.

`memory-agent-live-smoke` checks generated MCP configs only. It is strict for
MCP protocol, server readiness, token redaction, and default scope.
`memory-agent-live-smoke-agents` also launches real installed agent CLIs and
records Codex/Claude/Gemini/OpenCode evidence, but treats missing local model
auth as an advisory blocked state. Use `memory-agent-live-smoke-agents-strict`
...
passes `MEMORY_MCP_RUNTIME_*` overrides through `bin/memory-mcp`, so an
...
Gemini CLI may inject `wait_for_previous` into tool calls. Memory MCP ignores
...
`memory-agent-auth-doctor` runs plain model prompts without Memory MCP, so a 401
...
green. `memory-agent-auth-repair` is an interactive helper that runs the
official Claude and OpenCode login flows, then re-runs strict auth verification.
`memory-agent-install-doctor` treats plugin-kit-ai list/doctor failures as hard
