...
def test_generated_plugin_package_manifests_reference_existing_artifacts() -> None:
    ...
    for manifest_path in manifests:
        ...
        assert manifest["name"] == "memo-stack-agent-plugin"
...
def test_generated_plugin_hook_retrieves_memory_context_e2e(tmp_path: Path) -> None:
    with run_memo_stack_server(tmp_path) as server:
        ...
        completed = subprocess.run(
            [str(PLUGIN_ROOT / "bin" / "memo-stack-plugin-hook"), "UserPromptSubmit"],
            cwd=PLUGIN_ROOT,
            env=env,
...
def test_generated_gemini_hook_plugin_before_agent_context_e2e(tmp_path: Path) -> None:
    with run_memo_stack_server(tmp_path) as server:
        ...
        completed = subprocess.run(
            [
                str(GEMINI_HOOK_PLUGIN_ROOT / "bin" / "memo-stack-plugin-hook"),
                "GeminiBeforeAgent",
            ],
            cwd=GEMINI_HOOK_PLUGIN_ROOT,
            env=env,
...
def test_generated_plugin_hooks_reference_existing_runner() -> None:
    ...
    assert commands == {
        "./bin/memo-stack-plugin-hook UserPromptSubmit",
        "./bin/memo-stack-plugin-hook Stop",
    }
    assert (PLUGIN_ROOT / "bin" / "memo-stack-plugin-hook").exists()
...
def test_generated_gemini_hook_plugin_hooks_reference_existing_runner() -> None:
    ...
    assert commands == {
        "${extensionPath}${/}bin${/}memo-stack-plugin-hook GeminiSessionStart",
        "${extensionPath}${/}bin${/}memo-stack-plugin-hook GeminiBeforeAgent",
        "${extensionPath}${/}bin${/}memo-stack-plugin-hook GeminiAfterAgent",
...
def test_memory_agent_plugin_doctors_do_not_echo_token_e2e(tmp_path: Path) -> None:
    with run_memo_stack_server(tmp_path) as server:
        for plugin_root in DOCTOR_PLUGIN_ROOTS:
            ...
            completed = subprocess.run(
                [str(plugin_root / "bin" / "memo-stack-mcp-doctor")],
                cwd=plugin_root,
                env=env,
                check=True,
                capture_output=True,
