...
def test_memory_mcp_auth_failure_redaction_e2e(tmp_path: Path) -> None:
    with run_memo_stack_server(tmp_path, token="real-e2e-token") as server:
...
async def _run_mcp_lifecycle(base_url: str, token: str) -> None:
    ...
    async with (
        ...
        assert rejected_candidate["decision_code"] == "memo_stack_mcp.policy.secret_detected"
        ...
        assert duplicate_item["decision_code"] == "memo_stack_mcp.duplicate.existing_memory"
...
async def _run_mcp_policy_modes(base_url: str, token: str) -> None:
    ...
    async with (
        ...
        assert blocked.structuredContent["error"]["code"] == "memo_stack_mcp.policy.write_mode_off"
...
async def _run_mcp_auth_failure(base_url: str) -> None:
    ...
    async with (
        ...
        assert failed.structuredContent["error"]["code"] == "memo_stack_mcp.gateway.auth_failed"
