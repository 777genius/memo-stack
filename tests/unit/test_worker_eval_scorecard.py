    ...
    return {
        ...
        "quality-golden": {
            ...
            "metrics": {
                ...
                "no_candidate_abstention_rate": 1.0,
                "no_candidate_leak_count": 0,
...
def test_memory_quality_scorecard_fails_on_no_candidate_abstention_regression() -> None:
    suite_results = _scorecard_fixture_results()
    suite_results["quality-golden"]["metrics"]["no_candidate_abstention_rate"] = 0.5
    suite_results["quality-golden"]["metrics"]["no_candidate_leak_count"] = 1

    result = build_memory_quality_scorecard(suite_results)

    assert result["ok"] is False
    assert result["capabilities"]["retrieval_context_memory_layer"]["ok"] is False
    assert result["capabilities"]["retrieval_context_memory_layer"]["failed_checks"] == [
        "no_candidate_abstention_rate",
        "no_candidate_leak_count",
    ]
    assert result["metrics"]["quality_no_candidate_abstention_rate"] == 0.5
    assert result["metrics"]["quality_no_candidate_leak_count"] == 1
